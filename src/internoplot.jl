function internoplot(samp::Sample,
    ...
    x0, sx0, y0, sy0, rx0y0 = internochron(samp,method,fit)
    ...
    if isnothing(method)
        ...
        Pname, Dname, dname = unpack(method.ions)
        xlab = Pname * "/" * Dname
        ylab = dname * "/" * Dname
        p = internoplot(x0,y0,E,Phat,Dhat,dhat;
                        legend=legend,nsigma=nsigma,
                        xlab=xlab,ylab=ylab,plot_options...)
        ty0 = x0y02t(x0,y0,E,method.name)
        sdig = 2
        tdig = ceil(Int,log10(ty0.t/ty0.st)) + sdig
        tstring = "t = " *
            string(round(ty0.t,sigdigits=tdig)) * "±" *
